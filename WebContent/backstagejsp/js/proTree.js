(function($) {
			...
			Tree.prototype.proHTML = function(arr) {
				...
				arr.map(function(item) {
					...
					if(item.child && item.child.length > 0) {
						lihtml += "<a href='javascript:ovid()' class='title_name' ischek='true'><i  class='" + self.mouIconOpen + "'></i>" +
							"<span class='title'  id='" + item.id + "'>" + item.name +"("+item.amount+")"+"</span></a>"
						var _ul = self.proHTML(item.child);
						lihtml += _ul + "</li>";
					} else {
						lihtml += "<i class='" + self.simIcon + "'></i>" +
							"<span id='" + item.id + "'>" + item.name +"("+item.amount+")"+ "</span>";
					}
					ulHtml += lihtml;
				})
				ulHtml += "</ul>";
			...
			Tree.prototype.bindEvent = function() {
				var self = this;
				this.element.find(".menuUl li .title_name").click(function() {
					var ischek = $(this).attr("ischek");
					if(ischek == 'true') {
						var menuUl = $(this).closest("li").children(".menuUl");
						$(this).find("i").removeClass(self.mouIconOpen).addClass(self.mouIconClose)
						menuUl.hide();
						$(this).attr("ischek", 'false');
					} else if(ischek == 'false') {
						var menuUl = $(this).closest("li").children(".menuUl");
						menuUl.show();
						$(this).find("i").removeClass(self.mouIconClose).addClass(self.mouIconOpen)
